...
- **156 tests total, 0 failed, 0 skipped** across all three unit suites

## What is NOT Done

- **Task 6** (Playwright smoke suite v1): deferred per producer kickoff directive — out of scope
- Task 7 (sprint hardening): this document completes the dev-lane portion; producer merge review pending
...
| `src/TriTrainer.ApiService/Program.cs` | TargetValue guard, generation wiring, PATCH endpoint |
...
None open. One Razor syntax bug (RZ1010 `@{}` inside `else`) was found and fixed during Task 4.
...
3. **DisciplinePerformance goal with TargetValue ≤ 0 or missing** → `POST /v1/goals` must return 400.
4. **Plans page Activate button** — create a plan, confirm badge shows "Draft", click Activate, confirm badge updates to "Active" without page reload.
5. **Progress page zero-row filtering** — a discipline row with 0 planned AND 0 completed must not appear in the table.
6. **Sprint 1 regression** — `/activities` calendar, CRUD, and all Sprint 1 endpoints must remain unaffected.
